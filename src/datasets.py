...
from config import rgb_dirs, pose_dirs, pose_dirs_apple
...
def load_video_support_rgb(path, tmp):
    try:
        ...
        raise ImportError("Decord is not installed. Please install it for RGB support (note: Decord may have issues on Mac ARM64).")
...
class S2T_Dataset(Base_Dataset):
    def __init__(self, path, args, phase):
        ...
        # Use Apple Vision poses if specified
        use_apple = getattr(args, 'use_apple_pose', False)
        pose_source = pose_dirs_apple if use_apple else pose_dirs

        if self.args.dataset == "CSL_Daily":
            self.pose_dir = pose_source[args.dataset]
            self.rgb_dir = rgb_dirs[args.dataset]
            
        elif "WLASL" in self.args.dataset:
            self.pose_dir = os.path.join(pose_source[args.dataset], phase)
            self.rgb_dir = os.path.join(rgb_dirs[args.dataset], phase)
        
        elif self.args.dataset == "VSL":
            self.pose_dir = pose_source[args.dataset]
            self.rgb_dir = rgb_dirs[args.dataset]
